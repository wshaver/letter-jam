...
**Tech Stack:** React 18, Vite 5, TypeScript 5 (strict), Vitest 2 + jsdom + React Testing Library, canvas-confetti, browser `SpeechSynthesis`.
...
- **Wrong-answer default is `keepTrying`;** `oneAndDone` is opt-in per profile.
...
Run: `npm install`
Then: `npx vitest run src/smoke.test.ts`
...
- Produces:
  ...
  - `Box = 1|2|3|4|5`
  - `WordState { box: Box; seen: number; correct: number; introduced: boolean }`
...
- Produces:
  - `NUM_BOXES = 5`
  - `newWordState(): WordState` (box 1, introduced true, seen/correct 0)
  - `recordResult(state: WordState, correctFirstTry: boolean): WordState`
...
```ts
import { newWordState, recordResult, boxWeight, NUM_BOXES } from './leitner';

it('new words start in box 1, introduced', () => {
...
it('any wrong tap drops back to box 1', () => {
  let s = { box: 4 as const, seen: 3, correct: 3, introduced: true };
...
export function newWordState(): WordState {
  return { box: 1, seen: 0, correct: 0, introduced: true };
}

export function recordResult(state: WordState, correctFirstTry: boolean): WordState {
  const box = (correctFirstTry ? Math.min(state.box + 1, NUM_BOXES) : 1) as Box;
  return {
    box,
    seen: state.seen + 1,
...
```bash
git add -A
git commit -m "feat: add Leitner box engine"
```
...
- Test: `src/engine/similarity.test.ts`
...
```bash
git add -A
git commit -m "feat: add word similarity scoring"
```
...
it('upsertProfile adds then replaces by id', () => {
  const blob: SaveBlob = { version: 1, activeProfileId: null, profiles: [] };
  const p = createProfile('id1', 'Ada', '🦄');
  const b1 = upsertProfile(blob, p);
  expect(b1.profiles).toHaveLength(1);
  const b2 = upsertProfile(b1, { ...p, name: 'Ada B' });
  expect(b2.profiles).toHaveLength(1);
...
export function upsertProfile(blob: SaveBlob, profile: Profile): SaveBlob {
  const others = blob.profiles.filter((p) => p.id !== profile.id);
  return { ...blob, profiles: [...others, profile] };
}
...
### Task 7: Session progression & difficulty
...
**Interfaces:**
- Consumes: `newWordState` from `./leitner`; `Word`, `Grade`, `Profile`, `Difficulty` from `./types`.
- Produces:
  ...
  - `masteryFraction(profile: Profile): number`
  - `deriveDifficulty(profile: Profile): Difficulty`
...
it('difficulty rises with mastery', () => {
  const easy = deriveDifficulty(createProfile('id', 'A', '🦄'));
  expect(easy.choiceCount).toBe(3);
  expect(masteryFraction(createProfile('id', 'A', '🦄'))).toBe(0);
});
```
...
```ts
import type { Box, Difficulty, Grade, Profile, Word } from './types';
...
export function masteryFraction(profile: Profile): number {
  const states = Object.values(profile.progress.words).filter((s) => s.introduced);
  if (states.length === 0) return 0;
  const mastered = states.filter((s) => s.box >= DEFAULT_SESSION_CONFIG.masteredBox).length;
  return mastered / states.length;
}

export function deriveDifficulty(profile: Profile): Difficulty {
  const m = masteryFraction(profile);
  if (m < 1 / 3) return { choiceCount: 3, decoyNearness: 0.2 };
  if (m < 2 / 3) return { choiceCount: 4, decoyNearness: 0.5 };
  return { choiceCount: 5, decoyNearness: 0.8 };
}
...
```bash
git add -A
git commit -m "feat: add session progression, trickle, and difficulty"
```
...
**Interfaces:**
- Consumes: `boxWeight` from `./leitner`; `weightedPick`, `shuffle`, `Rng` from `./random`; `similarity` from `./similarity`; `Word`, `Profile`, `Difficulty`, `Round` from `./types`.
- Produces:
  - `activePool(profile: Profile, words: Word[]): Word[]`
  - `pickDecoys(target: Word, words: Word[], difficulty: Difficulty, rng: Rng): Word[]`
  - `buildRound(profile: Profile, words: Word[], difficulty: Difficulty, rng: Rng): Round`
...
it('buildRound returns exactly choiceCount unique choices including the target', () => {
  const p = profileWith(['cat']);
  const diff: Difficulty = { choiceCount: 3, decoyNearness: 0.5 };
  const round = buildRound(p, WORDS, diff, seeded(3));
...
export function buildRound(
  ...
  const pool = activePool(profile, words);
...
```bash
git add -A
git commit -m "feat: add round builder with difficulty-tiered decoys"
```
...
- Produces:
  ...
  - `createSpeaker(preferredName?: string): Speaker` (defaults to `'Google US English'`)
...
```bash
git add -A
git commit -m "feat: add SpeechSynthesis wrapper with voice selection"
```
...
**Interfaces:**
- Consumes: `introduceIfNeeded`, `deriveDifficulty` from `../engine/session`; `buildRound` from `../engine/roundBuilder`; `applyResult` from `../engine/profiles`; `Speaker` from `../engine/speech`; `Rng` from `../engine/random`; `Profile`, `Word`, `Round` from `../engine/types`.
- Produces:
...
import { deriveDifficulty, introduceIfNeeded } from '../engine/session';
...
export function useGame(opts: UseGameOpts) {
  ...
  const startRound = useCallback(() => {
    ...
    const difficulty = deriveDifficulty(introduced);
    const r = buildRound(introduced, words, difficulty, rng);
    setRound(r);
    setWrongIds(new Set());
    hadWrong.current = false;
    setCelebration(null);
    setStatus('playing');
    speaker.speak(r.target.text);
  }, [words, speaker, onProfileChange, rng]);

  useEffect(() => {
    startRound();
    // Start exactly one round on mount; subsequent rounds come from next().
  ...
  const replay = () => {
    if (round) speaker.speak(round.target.text);
  };
    ...
    <div className="play">
      ...
      <div className="cards">
        {round.choices.map((w) => (
          <button
            key={w.id}
            className={`card ${wrongIds.has(w.id) ? 'faded' : ''}`}
...
it('speaks the target and celebrates a first-try correct tap', async () => {
  ...
  const target = spoken[spoken.length - 1];
...
it('shows the small celebration after a wrong-then-correct tap', async () => {
  ...
  const target = spoken[spoken.length - 1];
  const wrong = WORDS.find((w) => w.id !== target)!;
  await user.click(screen.getByRole('button', { name: wrong.id }));
  await user.click(screen.getByRole('button', { name: target }));
  expect(screen.getByText('🎉 Nice!')).toBeInTheDocument();
});

it('ends the round on a wrong tap in oneAndDone mode', async () => {
  const user = userEvent.setup();
  const { spoken, speaker } = fakeSpeaker();
...
- Update: `src/smoke.test.ts` (App now loads asynchronously)
- Append styles: `src/index.css`
...
export function App() {
  ...
  const update = useCallback((next: SaveBlob) => {
    setBlob(next);
  ...
  if (screen === 'select' || !active) {
    return (
      <ProfileSelect
        blob={blob}
        onPick={(id) => {
          update({ ...blob, activeProfileId: id });
          setScreen('play');
        }}
        onCreate={(name, avatar) => {
          const p = createProfile(crypto.randomUUID(), name, avatar);
          update(upsertProfile({ ...blob, activeProfileId: p.id }, p));
          setScreen('play');
        }}
      />
    );
  ...
  const onProfileChange = (p: Profile) => update(upsertProfile(blob, p));
...
- [ ] **Step 5: Update `src/smoke.test.ts`** (App now renders "Loading…" then the select screen)
...
2. A word is spoken; tapping the correct card fires confetti + "Yay!"; tapping a wrong card first fades it and the eventual correct tap shows the smaller "Nice!".
3. The 🔊 button replays the word.
4. Settings (⚙️) toggles "stop after a wrong answer"; in that mode a wrong tap ends the round with "aw…".
5. Reload the page — the player and their progress persist.
6. Play many rounds — words that were tapped wrong recur sooner than mastered ones.
...
- **Difficulty (choices 3→5, far→near decoys):** Tasks 7 (`deriveDifficulty`), 8 (`pickDecoys`).
...
- **Multiple local profiles, single JSON blob behind `ProfileStore`:** Tasks 6, 12.
- **Celebration tiers (big / small / none):** Tasks 10 (`useGame`, `Feedback`).
- **keepTrying default, oneAndDone opt-in:** Tasks 10, 12.
- **TTS Google US English + fallback, isolated in speech.ts, tap-to-replay:** Tasks 9, 10.
- **Not a dexterity game:** no timers anywhere; motion is CSS-only.
